...
bool MCNK::Read(std::shared_ptr<Bytebuffer>& buffer, const ChunkHeader&, const WDT& wdt, ADT& adt)
{
    ZoneScoped;

    adt.cells.reserve(MAP_CELLS_PER_CHUNK);

    buffer->readData -= sizeof(ChunkHeader);
    ...
    for (i32 i = 0; i < MAP_CELLS_PER_CHUNK; i++)
    {
        ...
        size_t extraDataSize = header.size - sizeof(MCNK) + sizeof(ChunkHeader);
